...
def test_connected_calibration_fixture_run_writes_receipt_packet_and_intake(tmp_path: Path) -> None:
    ...
    assert all(entry["mcp_commands"]["source_answer"].startswith("aoa-course mcp call source_answer ") for entry in smoke_entries)
    ...
    assert all(f'"source_id":"{entry["source_id"]}"' in entry["mcp_commands"]["source_answer"] for entry in smoke_entries)
    assert all('"mode":"hybrid"' in entry["mcp_commands"]["answer"] for entry in smoke_entries)
    assert all('"mode":"hybrid"' in entry["mcp_commands"]["source_answer"] for entry in smoke_entries)
    ...
    assert all(entry["coverage"]["schema"] == "aoa_course_ingest_coverage_v1" for entry in sync_entries)
    assert all(entry["coverage"]["complete_for_scope"] is True for entry in sync_entries)
    assert all(entry["identity_continuity"]["status"] in {"initial", "stable"} for entry in sync_entries)
    assert all(entry["identity_continuity"]["stable_retention_rate"] == 1.0 for entry in sync_entries)
    assert all(entry["identity_continuity"]["removed_id_count"] == 0 for entry in sync_entries)
