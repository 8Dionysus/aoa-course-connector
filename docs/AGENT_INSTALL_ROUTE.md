...
10. Run the offline starter proof.
11. Run `preflight semantic-provider --run starter-fixture --require-ready`,
...
12. Register a Stepik course with `discover stepik 67 --register`, then run
...
13. Run `discover stepik-account --from-fixture --register --source-limit 1` to
   prove connected-account course discovery can write Stepik sources without
   live credentials.
14. Run `preflight live --platform stepik` to prove the live readiness report
...
15. Run `smoke stepik-fixture 67` to prove the combined clean API registration,
   sync, index/graph, answer, and privacy-safe report route.
16. Run browser fixture discovery with `--register` to prove the local source
   registry route.
17. After starter, Stepik fixture, and GetCourse browser fixture artifacts are
    built, run `eval answer-quality` to prove top-result path, source id,
    freshness, snippet, and evidence-field quality.
18. Run MCP calls for `connector_readiness`, `goal_audit`,
...
19. After a registry-backed Stepik fixture sync, run `refresh query
...
20. Run the freshness conflict fixture and `eval freshness-ranking` to prove
    current material ranks above stale material when base relevance is tied.
21. Run the authority conflict fixture and `eval authority-ranking` to prove
    official lessons and mentor comments rank above learner comments when base
    relevance is tied.
22. After Stepik, GetCourse, and Skillspace fixture indexes are built, run
    `eval adapter-authority` to prove adapter-derived authority metadata reaches
    normalized objects and query packets.
23. Run `eval browser-transcripts` to prove visible GetCourse/Skillspace
    transcript and caption text becomes canonical transcript objects and
    source-backed answer evidence.
24. Run `eval live-calibration` to prove the fixture-safe calibration packet for
    GetCourse, Skillspace, and Stepik smoke reports before collecting connected
    account reports.
25. Run `calibration connected-run --mode fixture --run connected-fixture-proof`
    ...
    live sources.
26. Before live browser sources, run `auth plan-browser-state`, capture the
...
27. Run `preflight live --platform getcourse` or
...
28. Confirm browser preflight marks only sources whose host matches the saved
    storage state as sync-ready.
29. Run `preflight connected-plan --write-runbook
...
30. In a large registry, add `--source-id "<registered-source-id>"` to
...
32. Add live sources only after auth-state and storage roots are local and
    ignored by Git.
