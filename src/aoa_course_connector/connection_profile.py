...
CLI_RUNBOOK_FLAG = "--write-runbook"
...
def render_connection_profile_runbook(inspection: dict[str, object]) -> str:
    """Render a redacted Markdown runbook from a connection profile inspection."""

    profile = inspection.get("profile") if isinstance(inspection.get("profile"), dict) else {}
    registry = inspection.get("source_registry") if isinstance(inspection.get("source_registry"), dict) else {}
    platforms = ", ".join([str(item) for item in profile.get("platforms", [])]) if isinstance(profile.get("platforms"), list) else ""
    lines = [
        "# Course Connection Profile Runbook",
        "",
        "This runbook is generated from `aoa_course_connection_profile_inspection_v1`.",
        "It is runtime operator state: keep it outside Git and do not paste private source refs into public issues.",
        "",
        "## Summary",
        "",
        f"- profile: `{profile.get('name')}`",
        f"- profile_path: `{profile.get('path')}`",
        f"- source_count: `{profile.get('source_count')}`",
        f"- platforms: `{platforms}`",
        f"- network_touched: `{bool(inspection.get('network_touched'))}`",
        f"- redacted: `{bool(inspection.get('redacted'))}`",
        f"- source_registry: `{registry.get('path')}`",
        f"- registered_profile_source_count: `{registry.get('registered_profile_source_count')}`",
        "",
        "## Sources",
        "",
    ]
    for source in _dict_items(inspection.get("sources")):
        lines.extend(
            [
                f"### {source.get('platform')} `{source.get('title')}`",
                "",
                f"- registered: `{bool(source.get('registered'))}`",
                f"- source_id: `{source.get('source_id')}`",
                f"- access_mode: `{source.get('access_mode')}`",
            ]
        )
        command = str(source.get("register_command") or "")
        if command:
            lines.extend(["", "Register:", "```bash", command, "```"])
        lines.append("")

    browser_auth = _dict_items(inspection.get("browser_auth"))
    if browser_auth:
        ...
        for item in browser_auth:
            lines.extend(
                [
                    f"### {item.get('platform')}",
                    "",
                    f"- state_ready: `{bool(item.get('state_ready'))}`",
                    f"- state_status: `{item.get('state_status')}`",
    ...
    if connected_plans:
        lines.extend(["## Connected Plans", ""])
        for plan in connected_plans:
            source_ids = ", ".join([str(item) for item in plan.get("source_ids", [])]) if isinstance(plan.get("source_ids"), list) else ""
            lines.extend(
                [
                    f"### {plan.get('platform')}",
                    "",
                    f"- registered: `{bool(plan.get('registered'))}`",
            ...
            if command:
                lines.extend(["", "Plan:", "```bash", command, "```"])
            lines.append("")
    ...
    if readiness:
        lines.extend(
            [
                ...
                f"- ready_for_connected_run: `{bool(readiness.get('ready_for_connected_run'))}`",
                f"- ready_for_semantic_build: `{bool(readiness.get('ready_for_semantic_build'))}`",
                f"- registered_source_count: `{readiness.get('registered_source_count')}` / `{readiness.get('source_count')}`",
                f"- browser_auth_ready_count: `{readiness.get('browser_auth_ready_count')}` / `{readiness.get('browser_source_count')}`",
                f"- ready_connected_plan_count: `{readiness.get('ready_connected_plan_count')}` / `{readiness.get('connected_plan_count')}`",
                "",
            ]
        )
        connected_run_commands = [str(item) for item in readiness.get("connected_run_commands", [])] if isinstance(readiness.get("connected_run_commands"), list) else []
        if connected_run_commands:
            lines.extend(["Connected run commands:"])
            for command in connected_run_commands:
                lines.extend(["```bash", command, "```", ""])
        blockers = [str(item) for item in readiness.get("blocked_by", [])] if isinstance(readiness.get("blocked_by"), list) else []
        if blockers:
            lines.extend(["Blocked by:"])
            lines.extend([f"- {blocker}" for blocker in blockers])
            lines.append("")

    semantic = inspection.get("semantic_provider") if isinstance(inspection.get("semantic_provider"), dict) else {}
    if semantic:
        lines.extend(
            [
                "## Semantic Provider",
                ...
                f"- ready: `{bool(semantic.get('ready'))}`",
                f"- status: `{semantic.get('status')}`",
                f"- network_touched: `{bool(semantic.get('network_touched'))}`",
                "",
            ]
        )
        commands = semantic.get("commands") if isinstance(semantic.get("commands"), dict) else {}
        for label in ["recheck", "build", "semantic_query", "hybrid_answer", "mcp_semantic_search"]:
            command = str(commands.get(label) or "")
            if command:
                lines.extend([label.replace("_", " ").title() + ":", "```bash", command, "```", ""])

    next_commands = [str(item) for item in inspection.get("next_commands", [])] if isinstance(inspection.get("next_commands"), list) else []
    if next_commands:
...
def write_connection_profile_runbook(inspection: dict[str, object], path: Path) -> dict[str, object]:
    target = path.expanduser()
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(render_connection_profile_runbook(inspection), encoding="utf-8")
    return {
        "schema": "aoa_course_connection_profile_runbook_v1",
        "status": "ok",
        "path": str(target),
        "written": True,
        "format": "markdown",
